-- Data Masking Functions (SHA512) (No Secret Key)
...
-- Helper function
-- Hashes column
CREATE OR REPLACE FUNCTION mask.hash(p_value text)
RETURNS text
LANGUAGE sql
IMMUTABLE
STRICT
...
STRICT
AS $$
SELECT lower(substr(mask.hash(p_email),1,16)) || '@example.test';
$$;
...
DECLARE
    ...
    n bigint;
BEGIN
    h := substr(mask.hash(p_phone),1,15);
    n := ('x' || h)::bit(60)::bigint;
...
SELECT lpad(
    (
        (('x'||substr(mask.hash(p_zip),1,8))::bit(32)::bigint % 90000)
        + 10000
    )::text,
    5,
    '0'
...
ORDER BY id
OFFSET (
    (('x'||substr(mask.hash(p_name),1,8))::bit(32)::bigint)
...
(
    (
        ('x'||substr(mask.hash(p_dob::text),1,8))::bit(32)::bigint % 3650
    ) - 1825
)::int;
$$;
